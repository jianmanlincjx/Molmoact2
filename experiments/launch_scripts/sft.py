...
SPATIAL_V4_MIXTURE = [
    ...
    ["spatial_detection", ["robopoint_detection"], 0.02],
    ["spatial_video", [
        "sims_vsi",
...
def _molmo2_general_er_mixture():
    ...
    video_pointing_tasks = [
        WeightedDataset("vixmo_points_minmax_0_5", sampling_rate=0.4, root_size_factor=1, message_weight=point_weight),
        WeightedDataset("vixmo_points_minmax_6_25", sampling_rate=0.3, root_size_factor=1, message_weight=point_weight),
        WeightedDataset("vixmo_points_minmax_26_60", sampling_rate=0.1, root_size_factor=1, message_weight=point_weight),
        WeightedDataset("academic_points_clip_63s_2fps", sampling_rate=0.2, root_size_factor=1, message_weight=point_weight),
    ]
    ...
    return [
        ["demo", [
            ...
            WeightedDataset(
                ...
                sampling_rate=1.5,
                message_weight=cap_weight,
            ),
        ], 0.15],
        ["video_academic", VIDEO_ACADEMIC_DATASETS, 0.20],
        ["image_academic", IMAGE_ACADEMIC_DATASETS, 0.25],
        ["video_pointing", video_pointing_tasks, 0.15],
        ["image_pointing", [
            WeightedDataset("pixmo_multi_points", root_size_factor=200000, message_weight=point_weight),
            WeightedDataset("pixmo_points_train", message_weight=point_weight, override_p_high_res=0.30),
            WeightedDataset("pixmo_count_train", message_weight=point_weight, override_p_high_res=0.30),
            WeightedDataset("pixmo_points_high_freq_train", message_weight=point_weight, override_p_high_res=0.30),
            WeightedDataset("cosyn_point", message_weight=point_weight, override_p_high_res=0.30),
        ], 0.25 - hardcode_weight],
        ["hardcodes", ["molmo2_hardcodes"], hardcode_weight],
    ]


def _scale_mixture(mixture, scale):
    return [[name, datasets, float(rate) * float(scale)] for name, datasets, rate in mixture]
...
def get_model(checkpoint, model):
    ...
    video_preprocessor_kwargs = dict(
        pooling_h=3,
        pooling_w=3,
...
def get_training_mixture(name):
    ...
    elif name == "spatial-all-v4":
        training_mixture = SPATIAL_V4_MIXTURE
    elif name.startswith("molmo2_embodied_spatial_mix_"):
        spatial_pct = int(name.rsplit("_", 1)[-1])
        if spatial_pct <= 0 or spatial_pct >= 100:
            raise ValueError(f"spatial percentage must be in (0, 100), got {spatial_pct}")
        spatial_ratio = spatial_pct / 100.0
        general_ratio = 1.0 - spatial_ratio
        training_mixture = (
            _scale_mixture(SPATIAL_V4_MIXTURE, spatial_ratio)
            + _scale_mixture(_molmo2_general_er_mixture(), general_ratio)
        )
