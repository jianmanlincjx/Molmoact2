...
from .object_tracking_utils import evaluate_video_point_tracking_with_masks,  evaluate_video_point_tracking_with_hota
from .point_tracking_utils import (
    PointTrackingParser, 
    create_pred_tracks_and_occlusions, evaluate_video_point_tracking
)
...
from ..preprocessing.point_formatter import extract_multi_image_points, extract_trajectories, \
    extract_points
...
class SavePredictions(Evaluator):
    ...
    def __init__(self, output_dir, json=True, save_tokens=True,
                 log_examples=10, table=100, output_file=None):
    ...
    def __call__(self, metadatas, predictions, tokenizer, step=None):
        ...
        for ex_ix, pred_seq in enumerate(new_tokens):
            ...
            if "points" in predictions:
                json_row["points"] = np.array(predictions["points"][ex_ix]).tolist()
...
class PointingEval(Evaluator):
    ...
    def __call__(self, metadatas, predictions, tokenizer, step=None):
        ...
        for ex_ix, pred in enumerate(model_predictions):
            ...
            if "points" in predictions:
                abs_preds = predictions["points"][ex_ix]
                if len(abs_preds):
                    abs_preds = abs_preds[:, 1:]
            else:
...
class RefExpEval:
    ...
    def __call__(self, metadatas, predictions, tokenizer, step=None):
        ...
        for ex_ix, pred_seq in enumerate(new_tokens):
            ...
            # boxes currently not supported
            raise NotImplementedError()
            x0, y0, w, h = metadata["bbox"]
            ref_box = [x0, y0, x0+w, y0+h]
            gt_bboxes.append([ref_box])
            if pred_box:
                iou, intersection, union = compute_iou(pred_box[0], ref_box, verbose=True)
                bboxes.append([pred_box[0]])
            else:
                bboxes.append(None)
                iou, intersection, union = 0, 0, 0
            scores.append(dict(acc=iou>0.5, miou=iou, iou_i=intersection, iou_u=union, valid=bool(pred_box)))

        out = {}
        for k in scores[0]:
            vals = [x[k] for x in scores if k in x]
            if k in ["iou_i", "iou_u"]:
                out[k] = sum_metric(vals)
            else:
                out[k] = mean_metric(vals)
        if self.n_to_log:
            score_to_log = [score["acc"] for score in scores]
            out["predictions"] = gather_examples_as_html(
                self.n_to_log, tokenizer, metadatas, predictions, score_to_log,
                pred_bboxes=bboxes, gt_bboxes=gt_bboxes,
...
class VideoObjectTrackingEval(Evaluator):
    ...
    def __call__(self, metadatas, predictions, tokenizer, step=None):
        new_tokens = predictions["predictions_text"]
        vocab = tokenizer
        ...
        for ex_ix, pred_seq in enumerate(new_tokens):
            ...
            example_id = example_id.partition("=")[0] # Remove any suffix after '='
            if "image_size" in metadata:
            ...
            gt_video_points_trajectory = metadata['points']
            ...
            pred_video_points_trajectory = extract_trajectories(
            ...
            video_metrics = evaluate_video_point_tracking_with_hota(
                pred_video_points_trajectory,
                gt_video_points_trajectory,
                metadata,
            )
            # Add to score lists
...
class VideoPointTrackingEval(Evaluator):
    """
    Evaluator for video point tracking tasks.

    Supported task name format:
        tap_davis_5_points_max_frames_60
        {dataset}_{num_points}_points[_max_frames_{max_frames}]
    """
    ...
    def __call__(self, metadatas, predictions, tokenizer, step=None):
        ...
        for ex_ix, pred_seq in enumerate(new_tokens):
            ...
            # Get ground truth
            # Ex: [{'frame': 0, 'time': '00:00.00', 'points': {0: {'point': [1038.0, 377.0], 'occluded': False}}}]
            query_points = metadata['query_points']
            gt_occluded = metadata['occluded']
            gt_tracks = metadata['target_points']

            # Decode prediction to match GT format
            pred = vocab.decode(pred_seq[pred_seq >= 0]).strip()
            video_fps = metadata['video_fps'] # Used for converting time to frame index
            pred_video_points_trajectory = PointTrackingParser.parse_prediction(
                pred, width, height, video_fps,
                format='video_point_track_all_frames_with_occlusion'
            )
            num_points, num_frames = gt_tracks.shape[:2]
            pred_occluded, pred_tracks = create_pred_tracks_and_occlusions(
                pred_video_points_trajectory, num_frames, num_points
            )

            try:
                # Evaluate video point tracking by checking point distances and using Hungarian algorithm.
                video_metrics = evaluate_video_point_tracking(
                    query_points,
                    gt_occluded,
                    gt_tracks,
                    pred_occluded,
                ...
                # Add to score lists
                for key in video_metrics.keys():
                    scores[key].append(video_metrics[key])
...
class CiderMetric(Metric):
    def __init__(self):
        ...
        try:
            ...
            raise ImportError("pycocoevalcap is not installed. Please install it with `pip install pycocoevalcap`.")

    def update(self, preds: List[str], gts: List[List[str]]):
        # preds: list[str], targets: list[list[str]] (multiple references)
        self.preds.extend(preds)
        self.gts.extend(gts)

    def tokenize(self, captions: Dict[str, List[str]]) -> Dict[str, List[str]]:
        try:
            tokenized_captions = self.tokenizer.tokenize(captions)
        except FileNotFoundError:
            raise FileNotFoundError("Java is not installed. Please install it with `conda install -y conda-forge::openjdk`.")
        return tokenized_captions

    def compute(self):
        preds = gather_object(self.preds)
        gts = gather_object(self.gts)

        preds = sum(preds, [])
        gts = sum(gts, [])

        gts = {str(i): [dict(caption=t) for t in gt.split("|||gt|||")] for i, gt in enumerate(gts)}
        preds = {str(i): [dict(caption=p)] for i, p in enumerate(preds)}

        gts = self.tokenize(gts)
        preds = self.tokenize(preds)

        score, _ = self.scorer.compute_score(gts, preds)
        return score


def cider_metric(preds: List[str], gts: List[List[str]]):
    metric = CiderMetric()
    metric.update(preds, gts)
    return metric


class CocoCaptionEval(Evaluator):
    ...
    def __call__(self, metadatas, predictions, tokenizer, step=None):
        ...
        for ex_ix, pred_seq in enumerate(new_tokens):
            ...
            pred = vocab.decode(pred_seq[pred_seq >= 0]).strip()
            preds.append(pred)
            gts.append("|||gt|||".join(metadata["captions"]))
            x0, y0, w, h = metadata["bbox"]
            ref_box = [x0, y0, x0+w, y0+h]
            gt_bboxes.append([ref_box])

        out = {"cider": cider_metric(preds, gts)}

        if self.n_to_log:
            out["predictions"] = gather_examples_as_html(self.n_to_log, vocab, metadatas, predictions, gt_bboxes=gt_bboxes)
...
class PointCloseEval(Evaluator):
    def eval_points(self, gt_points, model_points, image_size, time_factor):
        scores = dict()
        distances = None
        if len(gt_points) == 0:
            ...
            if len(model_points) == 0:
                ...
                if not isinstance(image_size[0], int):
                    correct_points = 0
                    all_dists = []
                    for i, (w, h) in enumerate(image_size):
                        thresh = max(math.sqrt(w*h)/self.thresh_factor, self.min_thresh)
                        img_gt = gt_points[gt_points[:, 0] == i]
                        img_model = model_points[model_points[:, 0] == i]
                        if len(img_gt) != 0 and len(img_model) != 0:
                            dists = cdist(img_gt[:, 1:], img_model[:, 1:])
                            row_ind, col_ind = linear_sum_assignment(dists)
                            distances = dists[row_ind, col_ind]
                            all_dists.append(distances)
                            correct_points += (distances < thresh).sum()
                else:
                    image_w, image_h = image_size
                    thresh = max(math.sqrt(image_w*image_h)/self.thresh_factor, self.min_thresh)
                    if gt_points.shape[-1] == 3:
                        gt_points = np.array(gt_points)
                        model_points = np.array(model_points)
                        dists = cdist(gt_points[:, 1:], model_points[:, 1:])
                        if time_factor is not None:
                            time_delta = gt_points[:, None, 0] - model_points[None, :, 0]
                            far_in_time = time_delta > time_factor
                            dists = np.where(far_in_time, 10000, dists,)
                    else:
                        dists = cdist(gt_points, model_points)
                    row_ind, col_ind = linear_sum_assignment(dists)
                    distances = dists[row_ind, col_ind]
                    if np.any(distances < 10000):
                        scores["dist"] = np.mean(distances[distances < 10000])
                    correct_points = (distances < thresh).sum()
                p = correct_points / len(gt_points)
                r = correct_points / len(model_points)
        scores["over_count"] = (len(model_points) >= 10) and (
            (len(gt_points) == 0 or len(model_points)) / len(gt_points) > 2.0)
        scores["recall"] = r
        scores["precision"] = p
        scores["f1"] = 0 if p == 0 else 2*p*r/(p+r)
        return scores

    def __init__(self, n_to_log=None, thresh_factor=15, time_factor=0.5, min_thresh=14):
        self.n_to_log = n_to_log
    ...
    def __call__(self, metadatas, predictions, tokenizer, step=None):
        ...
        for ix, (metadata, response) in enumerate(zip(metadatas, response_text)):
            ...
            if "image_group_size" in metadata:
                is_multi_point = True
                image_size = metadata["image_group_size"]
            else:
                is_video = "timestamps" in metadata
                image_size = metadata["image_size"]

            # Get ground truth points
            if is_video:
                if "gt_normalized_point_array" in metadata:
                    gt_points = np.array(metadata["gt_normalized_point_array"])
                    if len(gt_points) > 0:
                        gt_points[:, 1] *= image_size[0]/100.0
                        gt_points[:, 2] *= image_size[1]/100.0
                else:
                    gt_points = metadata["gt_point_array"]
            else:
                gt_points = metadata["points"]

            # Get model points
            if "points" in predictions:
                model_points = predictions["points"][ix]
                if len(model_points):
                    model_points = model_points[:, 1:]
                    if is_video or is_multi_point:
                        assert model_points.shape[-1] == 3
                    else:
                        assert model_points.shape[-1] == 2
            else:
                if is_video:
                    model_points = extract_multi_image_points(response, *metadata["image_size"])
                elif is_multi_point:
                    raise NotImplementedError()
                else:
                    model_points = extract_points(response, *metadata["image_size"])

            results = self.eval_points(gt_points, model_points, image_size, self.time_factor)

            for k, v in results.items():
                scores[k].append(v)
            if is_video:
                results = self.eval_points(gt_points, model_points, image_size, None)
                for k, v in results.items():
                    scores[k+"-notime"].append(v)
