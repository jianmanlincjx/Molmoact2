...
def get_evaluator(name) -> EvaluatorConfig:
    ...
    if name in [
    ...
    elif name == "vixmo_clip_counting":
        return EvaluatorConfig(vixmo_point_count_eval=True, point_close=True)
    elif name == "pixmo_multi_points":
        return EvaluatorConfig(point_close=True)
    elif name.startswith("point_at_the_square"):
        return EvaluatorConfig(point_close=True)
    elif name == "vixmo_frame_pointing":
        return EvaluatorConfig(frame_pointing_eval=True)
        ...
        return EvaluatorConfig(muir_bench_eval=True)
    elif name in [
        "mmsi_bench",
        ...
        return EvaluatorConfig(mmsi_bench_eval=True)
    elif name in [
        "mantis_eval",
        ...
        return EvaluatorConfig(blink_eval=True)
    ...
    if any(task in name for task in ["tap_davis"]):
        return EvaluatorConfig(
...
def get_default_max_tokens(name):
    ...
    elif name in ["point_bench"] or name.startswith("point_at_the_square") or name.startswith("pixmo_multi_points"):
        max_new_tokens = 128
