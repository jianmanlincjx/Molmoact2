...
from .evaluators import (
    ...
    VinogroundEval, VixMoCaptionEval, RefExpEval, CocoCaptionEval, CiderMetric, QVHighlightsEval,
    TomatoEval, TemporalBenchEval, Dream1KCaptionEval, MMEVideoOCREval, VideoHallucerEval,
    MuirBenchEval, BLINKEval, MMIUEval, LVBenchEval, MulSetEval, Ego3dBenchEval, VSIBenchEval,
    VideoObjectTrackingEval, VideoPointTrackingEval, VixMoPointCountEval, VixMoPointEval,
    PointBenchEval, MMSIBenchEval, PointCloseEval
)
...
class InfEvaluator:
    ...
    def __call__(self, predictions, example_metadata, tokenizer, device, step=None, **kwargs):
        ...
        for metric in self.metrics:
            ...
            elif isinstance(metric, RefExpEval):
                # RefExp has a macro-score that should be computed once we have
                # scores from all devices, that is overall IoU
                iou_i = resolved_metrics.pop("iou_i")
                iou_u = resolved_metrics.pop("iou_u")
                oiou = iou_i / (iou_u + 1e-6)
                resolved_metrics["oiou"] = oiou
...
class EvaluatorConfig(BaseConfig):
    ...
    def build(self, default_save_dir=None) -> InfEvaluator:
        ...
        elif self.point_close:
            evaluators.append(PointCloseEval(self.num_wandb_examples, thresh_factor=6, time_factor=1.0))
        ...
        elif self.refexp_eval:
            evaluators.append(RefExpEval(self.num_wandb_examples))
        elif self.coco_caption_eval:
            evaluators.append(CocoCaptionEval(self.num_wandb_examples))
        ...
        elif self.video_point_tracking_eval:
            evaluators.append(VideoPointTrackingEval(self.num_wandb_examples))
        ...
        if self.muir_bench_eval:
            evaluators.append(MuirBenchEval(self.num_wandb_examples))
        if self.blink_eval:
            evaluators.append(BLINKEval(self.num_wandb_examples))
        if self.mmiu_eval:
            evaluators.append(MMIUEval(self.num_wandb_examples))
        if self.mmsi_bench_eval:
            evaluators.append(MMSIBenchEval(self.num_wandb_examples))
        if self.mulset_eval:
