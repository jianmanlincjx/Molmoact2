...
def build_video_src(video_src, src_folder, clip=None):
    ...
    if clip is None:
        ...
        image_key = compute_hash(video_src + "|||" + str(clip))
        image_file = join(asset_dir, image_key)
        if not exists(image_file):
            logging.info(f"Adding {image_file} {clip} to asset cache")
            import ffmpeg
            input_file = ffmpeg.input(video_src).trim(start=clip[0], end=clip[1]).setpts('PTS-STARTPTS')
            output_file = ffmpeg.output(input_file, image_file, format='mp4')
            ffmpeg.run(output_file)
    src = f"/assets/{image_key}"
    return src


def build_video_asset(video_src, src_folder, clip=None):
    src = build_video_src(video_src, src_folder, clip)
    return f'<video style="max-height: 448px; max-width: 448px" controls><source src="{src}"></video>'
...
def example_to_html_dict(ex, preprocessor, show_patches=False, show_crops=False, asset_cache=None):
    ...
    if "image_paths" in metadata or "images" in metadata:
        ...
        if "video_path" in metadata and asset_cache is not None:
            ...
            pass # TODO show the frames in scrolling list of some sort
...
def build_html_table(data: List[Dict[str, Any]], col_widths=None, fixed_width=False, style=None) -> str:
