...
from typing import Tuple, Iterable, List, Set, Union

import PIL
from PIL import ImageFile, ImageOps, Image, ImageFilter
...
DEFAULT_IMAGE_PATH = os.environ.get("MOLMO_LEGACY_IMAGE_PATH", "/path/to/torch_datasets")
DEFAULT_MULTI_IMAGE_PATH = os.environ.get("MOLMO_LEGACY_MULTI_IMAGE_PATH", "/path/to/multi_image_datasets")
SUPPORTED_IMAGE_AUGMENTATION_MODES = ("none", "photometric", "full")


def normalize_image_augmentation_mode(value: Union[bool, str, None]) -> str:
    if isinstance(value, bool):
        return "photometric" if value else "none"
    if value is None:
        return "none"
    mode = str(value).strip().lower()
    if not mode:
        return "none"
    if mode not in SUPPORTED_IMAGE_AUGMENTATION_MODES:
        raise ValueError(
            f"Unsupported image augmentation mode '{mode}'. "
            f"Expected one of {sorted(SUPPORTED_IMAGE_AUGMENTATION_MODES)}."
        )
    return mode
...
class ImagePreprocessor:
    ...
    def maybe_augment_image(self, image, is_training, rng):
        if self.use_image_augmentation != "none" and is_training:
            return self._apply_augmentation(image, rng=rng)
        return image

    def build_single_crop(self, image, is_training, rng, image_size=None, apply_augmentation: bool = True):
        image_size = image_size or self.base_image_input_size
        if apply_augmentation:
            image = self.maybe_augment_image(image, is_training, rng)
    ...
    def build_overlapping_crops(self, image, is_training, rng, max_crops, overlap_margins, apply_augmentation: bool = True):
    ...
    def _apply_augmentation(self, image: np.ndarray, rng=None):
        ...
        augmentation_mode = normalize_image_augmentation_mode(self.use_image_augmentation)
        if augmentation_mode == "none":
            return image
        rng = rng if rng is not None else np.random

        # Convert inputs robustly into [0, 1] before any augmentation.
        # Supports uint8 [0,255], float [0,1], and float [0,255].
        if image.dtype == np.uint8:
            img_01 = image.astype(np.float32) / 255.0
        else:
            img = image.astype(np.float32)
            if img.max() > 1.0:
                img_01 = np.clip(img, 0.0, 255.0) / 255.0
            else:
                img_01 = np.clip(img, 0.0, 1.0)

        img_uint8 = np.clip(np.round(img_01 * 255.0), 0.0, 255.0).astype(np.uint8)

        pil_img = Image.fromarray(img_uint8)

        if augmentation_mode == "full":
            # Dynamic crop and rotation are geometric augmentations; keep them out of photometric-only mode.
        ...
        # torchvision's PIL blur path uses reflect padding, which requires padding < image dimension.
        # Tiny images can appear in some multi-image datasets, so skip blur for those edge cases.
        if min(aug_img.size) > blur_padding:
            blur = T2.RandomApply(
                [T2.GaussianBlur(kernel_size=blur_kernel_size, sigma=(0.1, 1.0))],
                p=0.2,
            )
            aug_img = blur(aug_img)
        ...
        if self.normalize_on_gpu:
            # Preserve the non-aug branch contract: downstream SigLIP normalization expects
            # uint8 [0, 255] when normalize_on_gpu=True.
            aug_uint8 = np.clip(np.round(aug_np * 255.0), 0.0, 255.0).astype(np.uint8)
            return aug_uint8
        return aug_np.astype(np.float32, copy=False)
