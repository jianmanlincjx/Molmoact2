#!/usr/bin/env bash
# Goal-pose prior v4 — Stage1: vision-free EE prior with 8 SE3 goal tokens.
#
# Same recipe as v3 Stage1 except:
#   - NUM_GOAL_TOKENS=8 (aligned with Stage2 pose-only width)
#   - OUTPUT_DIR under libero_goal_prior_v4/
# Reuses v3-corrected QUANTILES (does not rewrite stats).
#
# Requires: bash scripts/libero_goal_prior_v3/fix_stats.sh (once)
set -euo pipefail
...
export JOB_NAME="${JOB_NAME:-molmoact2-goalprior-stage1-v4}"
export OUTPUT_DIR="${OUTPUT_DIR:-${WS}/lerobot/outputs/libero_goal_prior_v4/seed_${SEED}/stage1}"
...
# v4: unify Stage1/Stage2 width at 8 pose tokens (v3 Stage1 used 4).
NUM_GOAL_TOKENS="${NUM_GOAL_TOKENS:-8}"
...
print(f"[v4-stage1] verified NEW state Z q01/q99=[{z_q01:.4f}, {z_q99:.4f}]")
PY
fi
...
echo "[v4-stage1] OUTPUT_DIR=${OUTPUT_DIR}"
echo "[v4-stage1] NUM_GOAL_TOKENS=${NUM_GOAL_TOKENS}"
echo "[v4-stage1] stats note: ${STATS_NOTE}"
