#!/usr/bin/env bash
# Goal-pose prior v2b (scheme-2a) — two functionally specialized latent groups.
#
# Same recurrent semantic -> visual aggregator as v2, but the 100 latent tokens are
# split into a goal-pose group (first NUM_SEMANTIC_VISUAL_POSE_TOKENS) and a context
# group. Only the pose group is supervised by L_pose, decoded via the v1-style concat
# head (no single-vector pooling). All 100 tokens still condition the action expert.
set -euo pipefail
...
export JOB_NAME="${JOB_NAME:-molmoact2-goalprior-stage2-v2b}"
...
export OUTPUT_DIR="${OUTPUT_DIR:-${WS}/lerobot/outputs/libero_goal_prior_v2b/seed_${SEED}/stage2}"
...
# Pose is now a strong auxiliary/prior (concat readout over the 8 pose tokens), not the
# dominant objective: down-weight it so the optimizer is not dragged by the harder term.
