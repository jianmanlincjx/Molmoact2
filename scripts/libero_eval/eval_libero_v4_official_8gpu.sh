#!/usr/bin/env bash
# Official 50-episode LIBERO evaluation for V4, sharded over eight GPUs.
#
# Each suite is split into task_ids 0-4 and 5-9. This preserves the official
# 50 episodes per task and suite horizons while halving the per-GPU task count.
set -euo pipefail

WS="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
POLICY_PATH="${1:-${POLICY_PATH:-}}"
if [[ -z "${POLICY_PATH}" || ! -f "${POLICY_PATH}/config.json" ]]; then
  echo "Usage: bash $0 /path/to/v4/checkpoint/pretrained_model" >&2
  exit 2
fi
...
if [[ "${DRY_RUN:-false}" == "true" ]]; then
  ...
  MODEL_LABEL="${MODEL_LABEL}" \
    bash "${WS}/scripts/libero_eval/eval_libero_v4_checkpoint.sh" "${POLICY_PATH}"
  exit 0
fi
