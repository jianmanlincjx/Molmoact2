...
DEFAULT_POLICY_PATH="${WS}/lerobot/outputs/libero_goal_prior_v2/seed_1000/stage2/checkpoints/010000/pretrained_model"
POLICY_PATH="${1:-${POLICY_PATH:-${DEFAULT_POLICY_PATH}}}"
if [[ -z "${POLICY_PATH}" || ! -f "${POLICY_PATH}/config.json" ]]; then
  echo "Goal-pose v2 checkpoint not found: ${POLICY_PATH}" >&2
...
if mismatches:
    raise SystemExit(
        "Refusing to evaluate: checkpoint is not Goal-Pose Prior v2:\n  "
        + "\n  ".join(mismatches)
    )
print(
    "[eval] verified v2:"
...
CHECKPOINT_LABEL="${CHECKPOINT_LABEL:-goal_prior_v2_${checkpoint_step}}"
